...
把数据集放到共享目录下的方法：请每位用户自行以自己分配到的集群命名空间为名建立文件夹（例如命名空间为`zhangsan1`则建立`share/zhangsan1`）。通过直接挂载NFS共享盘的方式参考`ubuntu-tf-nfs-direct-example.yaml`。
...
挂载本地磁盘参考`ubuntu-tf-local-disk-example.yaml`。需要同时申请持久化存储并挂载本地磁盘，可参考`ubuntu-tf-local-disk+nfs-pvc-example`。
