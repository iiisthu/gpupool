...
把数据集放到共享目录下的方法：请每位用户自行以自己分配到的集群命名空间为名建立文件夹（例如命名空间为`zhangsan1`则建立`share/zhangsan1`），公开数据集请放入`share/datasets/`下，并附带数据集的原始地址及说明文件。通过直接挂载NFS共享盘的方式参考`ubuntu-tf-nfs-direct-example.yaml`。
